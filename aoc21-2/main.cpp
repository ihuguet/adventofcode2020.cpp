...
static std::map<std::string,std::string> allergens_ingredients;
...
int main () {
    ...
    std::cout << "Ingredients with allergens: ";
    bool comma = false;
    for (auto it = allergens_ingredients.cbegin(); it != allergens_ingredients.cend(); it++) {
        if (comma) std::cout << ',';
        std::cout << it->second;
        comma = true;
    }
    ...
    while (allergens_possible_ingredients.size() > 0) {
        ...
        while (aller_it != allergens_possible_ingredients.end()) {
            ...
            if (ingrs.size() == 1) {
                ...
                allergens_ingredients[aller] = ingr;
